...
is_gum_raw_mode_failure() {
    local err_log="$1"
    [[ -s "$err_log" ]] || return 1
    grep -Eiq 'setrawmode|ioctl|inappropriate' "$err_log"
}
...
run_npm_global_install() {
    ...
    if [[ -n "$GUM" ]] && gum_is_tty; then
        ...
        local gum_err
        gum_err="$(mktempfile)"
        if "$GUM" spin --spinner dot --title "Installing OpenClaw package" -- bash -c "${cmd_quoted}>${log_quoted} 2>&1" 2>"$gum_err"; then
            return 0
        fi
        if is_gum_raw_mode_failure "$gum_err"; then
            GUM=""
            GUM_STATUS="skipped"
            GUM_REASON="gum raw mode unavailable"
            ui_warn "Spinner unavailable in this terminal; showing npm output directly"
            # Fall through to tee path below
        else
            if [[ -s "$gum_err" ]]; then
                cat "$gum_err" >&2
            fi
    ...
    # No spinner — show output directly so user can see progress
    ui_info "Running npm install (this may take a few minutes)..."
    "${cmd[@]}" 2>&1 | tee "$log"
}
...
install_openclaw_npm() {
    ...
    if ! run_npm_global_install "$spec" "$log"; then
        ...
        if [[ "$VERBOSE" != "1" ]]; then
            if [[ ! -s "$log" ]]; then
                ui_warn "npm produced no output (log is empty); error details unavailable"
                ui_info "Re-run with --verbose to see full npm output"
            elif [[ "$attempted_build_tool_fix" == "true" ]]; then
...
# ── Network: pre-flight mirror setup ─────────────────────
# Detect whether we need Chinese mirrors and set them up.
# Respects any mirrors already set via environment variables (e.g. from Tauri app).
setup_network_mirrors() {
    # If Homebrew mirrors are already set by the parent process, skip auto-detection
    if [[ -n "${HOMEBREW_BREW_GIT_REMOTE:-}" ]]; then
        ui_info "Using pre-configured network mirrors"
        return 0
    fi

    # Quick probe: can we reach GitHub within 5 seconds?
    ...
    # GitHub unreachable — check if we have ANY network by probing Chinese mirrors
    local mirror_reachable=false
    if command -v curl &>/dev/null; then
        if curl -fsSL --max-time 5 --head "https://registry.npmmirror.com/" &>/dev/null 2>&1; then
            mirror_reachable=true
        fi
    elif command -v wget &>/dev/null; then
        if wget -q --spider --timeout=5 "https://registry.npmmirror.com/" &>/dev/null 2>&1; then
            mirror_reachable=true
        fi
    fi

    if [[ "$mirror_reachable" == "false" ]]; then
        NO_NETWORK=1
        ui_warn "No network connectivity detected (GitHub and mirrors both unreachable)"
        ui_info "The installer will check for pre-installed tools and skip downloads"
        return 0
    fi

    ui_warn "GitHub appears unreachable; configuring Chinese mirrors"

    # Homebrew mirrors (TUNA)
    ...
    # Avoid slow auto-update
    export HOMEBREW_NO_AUTO_UPDATE=1
}

# ── Homebrew helpers ─────────────────────────────────────

# Try to locate the brew binary at all known locations and add it to PATH.
# Returns 0 if brew becomes available, 1 otherwise.
ensure_brew_on_path() {
    # Already on PATH?
    ...
    # Try known Homebrew install locations
    ...
    for candidate in "${brew_candidates[@]}"; do
        if [[ -x "$candidate" ]]; then
            ...
            # Also explicitly prepend the directory in case shellenv didn't work
    ...
    # Last resort: blindly add common paths
...
install_homebrew() {
    if [[ "$OS" == "macos" ]]; then
        if ! command -v brew &> /dev/null; then
            # Maybe brew IS installed but not on PATH (fresh terminal, non-login shell)
            if ensure_brew_on_path; then
                ui_success "Homebrew already installed (added to PATH)"
                return 0
            fi

            # No network — cannot download Homebrew; tell user what to pre-install
            if [[ "$NO_NETWORK" == "1" ]]; then
                ui_warn "Homebrew not found and no network available"
                ui_info "Please pre-install Node.js 22+ and npm on this machine"
                ui_info "  Download: https://nodejs.org/en/download/"
                ui_info "Then re-run this installer"
                exit 1
            fi

            if ! is_macos_admin_user; then
                ui_warn "Current user is not a macOS admin; skipping Homebrew install"
                ui_info "Will attempt Node.js direct binary install instead (no admin required)"
                # Return without installing Homebrew — install_node will use binary path
                return 0
            fi
            ui_info "Homebrew not found, installing"

            # Set NONINTERACTIVE so the Homebrew installer doesn't wait for RETURN
            export NONINTERACTIVE=1

            # Download Homebrew install script: try official URL, then mirrors.
            # Always include mirrors as fallback — raw.githubusercontent.com is often
            # blocked even when github.com itself is reachable.
            local brew_tmp
            brew_tmp="$(mktempfile)"
            local brew_urls=(
                "https://raw.githubusercontent.com/Homebrew/install/HEAD/install.sh"
                "https://mirrors.ustc.edu.cn/misc/brew-install.sh"
                "https://cdn.npmmirror.com/binaries/homebrew/install/HEAD/install.sh"
            )

            local brew_downloaded=false
            for brew_url in "${brew_urls[@]}"; do
                if download_file "$brew_url" "$brew_tmp" 2>/dev/null; then
                    brew_downloaded=true
                    ui_info "Downloaded Homebrew installer from ${brew_url}"
                    break
                fi
                ui_warn "Failed to download from ${brew_url}, trying next..."
            done

            if [[ "$brew_downloaded" != "true" ]]; then
                ui_error "Cannot download Homebrew install script"
                ui_info "All URLs attempted: ${brew_urls[*]}"
                ui_info "Install Homebrew manually: https://brew.sh"
                exit 1
            fi

            # Run Homebrew installer directly (NOT via run_quiet_step) because
            # the installer needs terminal access for sudo password prompt.
            ui_info "Running Homebrew installer (may prompt for password)..."
            if ! /bin/bash "$brew_tmp"; then
                ui_error "Homebrew installation failed"
                ui_info "Try installing Homebrew manually: https://brew.sh"
                exit 1
            fi

            # Robustly locate and activate brew after installation
            if ! ensure_brew_on_path; then
                ui_error "Homebrew installation completed but 'brew' command not found"
                ui_info "Checked: /opt/homebrew/bin/brew, /usr/local/bin/brew"
                ui_info "Try installing Homebrew manually: https://brew.sh"
                exit 1
            fi
...
# Append a PATH line to the user's shell config files.
# Strategy:
#   1. Determine the user's PRIMARY rc file based on $SHELL (zsh→.zshrc, bash→.bash_profile on macOS/.bashrc on Linux)
#   2. Always write to the primary rc file (create it if missing)
#   3. Also write to any OTHER existing rc files for cross-shell compatibility
# Usage: persist_path_to_rc <grep_pattern> <path_line>
#   grep_pattern: fixed-string pattern for dedup check (e.g. '.local/node/bin')
#   path_line:    the export line to append (single-quoted, unexpanded)
persist_path_to_rc() {
    local grep_pattern="$1"
    local path_line="$2"

    # Determine primary rc file based on user's actual shell
    local primary_rc=""
    ...
    # All candidate rc files (primary will be handled separately)
    local -a candidates=("$HOME/.bash_profile" "$HOME/.bashrc" "$HOME/.zshrc" "$HOME/.profile")

    # Step 1: Ensure primary rc file is written (create if needed)
    if ! grep -Fq "$grep_pattern" "$primary_rc" 2>/dev/null; then
        echo "$path_line" >> "$primary_rc"
    fi

    # Step 2: Also write to any OTHER existing rc files for cross-shell compat
    for rc in "${candidates[@]}"; do
        [[ "$rc" == "$primary_rc" ]] && continue
        if [[ -f "$rc" ]] && ! grep -Fq "$grep_pattern" "$rc"; then
            echo "$path_line" >> "$rc"
        fi
    done
}

# Install Node.js from prebuilt binary (bypasses Homebrew, no sudo needed)
# Downloads from Chinese mirror (npmmirror CDN) with official fallback.
install_node_from_binary() {
    local arch=""
    case "$(uname -m)" in
        arm64|aarch64) arch="arm64" ;;
        x86_64|amd64) arch="x64" ;;
        *) ui_warn "Unsupported architecture for binary install: $(uname -m)"; return 1 ;;
    esac
    ...
    case "$(uname -s)" in
        ...
        Linux) os_name="linux" ;;
        *) ui_warn "Unsupported OS for binary install: $(uname -s)"; return 1 ;;
    esac

    # Resolve latest v22.x version from mirror
    local node_version=""
    local version_urls=(
    ...
    local shasums_downloaded=false
    for vurl in "${version_urls[@]}"; do
        if download_file "$vurl" "$shasums_tmp" 2>/dev/null; then
            shasums_downloaded=true
            break
        fi
    done
    if [[ "$shasums_downloaded" != "true" ]]; then
        ui_warn "Cannot resolve Node.js version from mirror"
        return 1
    fi

    # Parse version from SHASUMS file (e.g. "node-v22.14.0-darwin-arm64.tar.gz")
    node_version="$(grep -oE 'node-v22\.[0-9]+\.[0-9]+' "$shasums_tmp" | head -1 | sed 's/node-//')"
    if [[ -z "$node_version" ]]; then
        ui_warn "Cannot parse Node.js version from SHASUMS"
        return 1
    fi

    local tarball="node-${node_version}-${os_name}-${arch}.tar.gz"
    local download_urls=(
        "https://cdn.npmmirror.com/binaries/node/${node_version}/${tarball}"
    ...
    for durl in "${download_urls[@]}"; do
        ui_info "Downloading Node.js ${node_version} from ${durl}..."
        if download_file "$durl" "$node_tmp" 2>/dev/null; then
            downloaded=true
            break
        fi
        ui_warn "Download failed, trying next mirror..."
    done

    if [[ "$downloaded" != "true" ]]; then
        ui_warn "Cannot download Node.js binary"
        return 1
    fi

    # Verify checksum
    local expected_sha=""
    expected_sha="$(grep "${tarball}" "$shasums_tmp" | awk '{print $1}')"
    if [[ -n "$expected_sha" ]]; then
        local actual_sha=""
        if command -v sha256sum >/dev/null 2>&1; then
            actual_sha="$(sha256sum "$node_tmp" | awk '{print $1}')"
        elif command -v shasum >/dev/null 2>&1; then
            actual_sha="$(shasum -a 256 "$node_tmp" | awk '{print $1}')"
        fi
        if [[ -n "$actual_sha" && "$actual_sha" != "$expected_sha" ]]; then
            ui_error "Node.js binary checksum mismatch"
            return 1
        fi
    fi

    # Extract to ~/.local/node
    local node_dir="$HOME/.local/node"
    rm -rf "$node_dir" 2>/dev/null || true
    mkdir -p "$node_dir"
    if ! tar -xzf "$node_tmp" -C "$node_dir" --strip-components=1 >/dev/null 2>&1; then
        ui_error "Failed to extract Node.js binary"
        rm -rf "$node_dir"
        return 1
    fi

    # Verify extracted binary works
    if [[ ! -x "$node_dir/bin/node" ]]; then
        ui_error "Node.js binary not found after extraction"
        rm -rf "$node_dir"
        return 1
    fi
    ...
    # Persist in shell profiles (including .bash_profile for macOS login shells)
    # shellcheck disable=SC2016
    persist_path_to_rc '.local/node/bin' 'export PATH="$HOME/.local/node/bin:$PATH"  # Added by OpenClaw installer'

    ui_success "Node.js ${node_version} installed to ${node_dir} (no Homebrew/sudo needed)"
    print_active_node_paths || true
    return 0
}
...
# Install Node.js
install_node() {
    # No network — cannot download Node.js from anywhere
    if [[ "$NO_NETWORK" == "1" ]]; then
        ui_error "Node.js v22+ is required but not installed, and no network is available"
        ui_info "Please pre-install Node.js 22+ on this machine before running the installer"
        ui_info "  Download offline: https://nodejs.org/en/download/"
        exit 1
    fi

    if [[ "$OS" == "macos" ]]; then
        # Strategy: try direct binary download first (no Homebrew/sudo needed),
        # fall back to Homebrew only if binary install fails.
        local node_installed=false

        ui_info "Trying direct Node.js binary download (no Homebrew/sudo needed)..."
        if install_node_from_binary; then
            node_installed=true
        else
            ui_info "Direct binary install failed; trying Homebrew..."
            # Verify brew is available before trying to use it
            if ! command -v brew &>/dev/null; then
        ...
        if [[ "$node_installed" != "true" ]]; then
            ui_error "Could not install Node.js via binary download or Homebrew"
            ui_info "Options to fix this:"
            ui_info "  1) Install Node.js 22+ manually: https://nodejs.org/en/download/"
            ui_info "  2) Install Homebrew first: https://brew.sh (requires admin)"
            ui_info "  3) Ask an admin to install Node.js or Homebrew for you"
            exit 1
        fi
        ui_success "Node.js installed"
        print_active_node_paths || true
    elif [[ "$OS" == "linux" ]]; then
        # Strategy: try direct binary download first (no sudo needed),
        # fall back to NodeSource + package manager if binary install fails.
        local node_installed=false

        ui_info "Trying direct Node.js binary download (no sudo needed)..."
        if install_node_from_binary; then
            node_installed=true
        else
            ui_info "Direct binary install failed; trying NodeSource..."
            require_sudo
            ...
            if command -v apt-get &> /dev/null; then
                local tmp
                tmp="$(mktempfile)"
                local nodesource_deb_urls=(
                    "https://deb.nodesource.com/setup_22.x"
                    "https://mirrors.tuna.tsinghua.edu.cn/nodesource/deb/setup_22.x"
                    "https://mirrors.ustc.edu.cn/nodesource/deb/setup_22.x"
                )
                local ns_downloaded=false
                for ns_url in "${nodesource_deb_urls[@]}"; do
                    if download_file "$ns_url" "$tmp" 2>/dev/null; then
                        ns_downloaded=true
                        ui_info "Downloaded NodeSource setup from ${ns_url}"
                        break
                    fi
                    ui_warn "NodeSource URL failed: ${ns_url}, trying next..."
                done
                if [[ "$ns_downloaded" != "true" ]]; then
                    ui_warn "All NodeSource URLs failed; skipping NodeSource setup"
                ...
                local tmp
                tmp="$(mktempfile)"
                local nodesource_rpm_urls=(
                    "https://rpm.nodesource.com/setup_22.x"
                    "https://mirrors.tuna.tsinghua.edu.cn/nodesource/rpm/setup_22.x"
                    "https://mirrors.ustc.edu.cn/nodesource/rpm/setup_22.x"
                )
                local ns_downloaded=false
                for ns_url in "${nodesource_rpm_urls[@]}"; do
                    if download_file "$ns_url" "$tmp" 2>/dev/null; then
                        ns_downloaded=true
                        ui_info "Downloaded NodeSource setup from ${ns_url}"
                        break
                ...
                local tmp
                tmp="$(mktempfile)"
                local nodesource_yum_urls=(
                    "https://rpm.nodesource.com/setup_22.x"
                    "https://mirrors.tuna.tsinghua.edu.cn/nodesource/rpm/setup_22.x"
                    "https://mirrors.ustc.edu.cn/nodesource/rpm/setup_22.x"
                )
                local ns_downloaded=false
                for ns_url in "${nodesource_yum_urls[@]}"; do
                    if download_file "$ns_url" "$tmp" 2>/dev/null; then
                        ns_downloaded=true
                        ui_info "Downloaded NodeSource setup from ${ns_url}"
                        break
...
detect_nvm_and_warn() {
    ...
    if [[ -n "$node_path" && "$node_path" == *".nvm"* ]]; then
        ...
        if [[ -n "$major" && "$major" -lt 22 ]]; then
            ui_warn "NVM detected with old Node ${current_version}; auto-switching to Node 22"
            # Source nvm so its commands are available in this shell
            # shellcheck disable=SC1090
            if [[ -f "${nvm_dir}/nvm.sh" ]]; then
                \. "${nvm_dir}/nvm.sh" --no-use 2>/dev/null || true
            fi
            if command -v nvm &>/dev/null || [[ -f "${nvm_dir}/nvm.sh" ]]; then
                # Install + switch + set as default — all non-interactive
                (
                    # shellcheck disable=SC1090
                ...
                # Re-source and update PATH so the new node is visible immediately
                # shellcheck disable=SC1090
                [[ -f "${nvm_dir}/nvm.sh" ]] && \. "${nvm_dir}/nvm.sh" 2>/dev/null || true
                export NVM_DIR="$nvm_dir"
                refresh_shell_command_cache
                local new_major
                new_major="$(node_major_version 2>/dev/null || true)"
                if [[ -n "$new_major" && "$new_major" -ge 22 ]]; then
                    ui_success "NVM switched to Node $(node -v 2>/dev/null || echo '22+')"
                    return 0
                fi
            fi
            # NVM switch failed — fall through to binary install path (node will be installed separately)
            ui_warn "NVM auto-switch failed; will use direct binary install"
        fi
    fi
}
...
fix_npm_permissions() {
    ...
    # shellcheck disable=SC2016
    persist_path_to_rc '.npm-global/bin' 'export PATH="$HOME/.npm-global/bin:$PATH"  # Added by OpenClaw installer'
...
ensure_openclaw_bin_link() {
    ...
    if [[ ! -x "${npm_bin}/openclaw" ]]; then
        # Make the entry.js executable (needs shebang + exec permission)
        chmod +x "$npm_root/openclaw/dist/entry.js" 2>/dev/null || true
...
ensure_user_local_bin_on_path() {
    ...
    # shellcheck disable=SC2016
    persist_path_to_rc '/.local/bin:' 'export PATH="$HOME/.local/bin:$PATH"  # Added by OpenClaw installer'
}
...
warn_shell_path_missing_dir() {
    ...
    echo "  Fix: add to your shell config (zsh: ~/.zshrc, bash: ~/.bash_profile or ~/.bashrc):"
...
resolve_openclaw_bin() {
    ...
    # Check user-local shim location
...
install_openclaw_from_git() {
    ...
    # If GitHub is unreachable (mirrors configured), try mirror for git clone
    if [[ -n "${HOMEBREW_BREW_GIT_REMOTE:-}" ]]; then
        # GitHub likely blocked; try ghproxy or direct (user may have proxy)
        ui_info "GitHub mirrors detected; git clone may be slow if no proxy is configured"
    fi
...
install_openclaw() {
    ...
    if ! install_openclaw_npm "${install_spec}"; then
        ...
        cleanup_npm_openclaw_paths
        if ! install_openclaw_npm "${install_spec}"; then
            ui_error "npm install failed after retry; giving up"
            print_active_node_paths || true
            exit 1
        fi
    fi

    # Try to fix the bin link before checking if the binary is resolvable
    ensure_openclaw_bin_link || true

    if [[ "${OPENCLAW_VERSION}" == "latest" && "${package_name}" == "openclaw" ]]; then
        if ! resolve_openclaw_bin &> /dev/null; then
            ui_warn "npm install openclaw@latest may not have created a working binary; retrying openclaw@next"
            cleanup_npm_openclaw_paths
...
# Remove exact duplicate lines from shell rc files (preserves first occurrence, order)
# Protects against external tools (openclaw doctor, nvm, etc.) appending duplicate init lines
deduplicate_rc_files() {
    for rc in "$HOME/.bash_profile" "$HOME/.bashrc" "$HOME/.zshrc" "$HOME/.profile"; do
        [[ -f "$rc" ]] || continue
        local original_lines
        original_lines="$(wc -l < "$rc")"
        # awk '!seen[$0]++' keeps first occurrence of each line, removes subsequent duplicates
        local tmp="${rc}.openclaw-dedup.tmp"
        if awk '!seen[$0]++' "$rc" > "$tmp" 2>/dev/null; then
            local deduped_lines
            deduped_lines="$(wc -l < "$tmp")"
            if [[ "$deduped_lines" -lt "$original_lines" ]]; then
                local removed=$(( original_lines - deduped_lines ))
                mv "$tmp" "$rc"
                ui_info "Cleaned ${removed} duplicate line(s) from $(basename "$rc")"
            else
                rm -f "$tmp"
            fi
        else
...
main() {
    ...
    # Set up network mirrors if GitHub is unreachable (China-friendly)
    setup_network_mirrors
    ...
    # Step 1+2: Node.js (tries direct binary download first, Homebrew as fallback)
    if ! check_node; then
        install_node
    fi
    ...
    # If no network and openclaw is not already installed, fail early with clear message
    if [[ "$NO_NETWORK" == "1" ]]; then
        if [[ -z "$(type -P openclaw 2>/dev/null || true)" ]]; then
            ui_error "Cannot install OpenClaw: no network connectivity"
            ui_info "This machine has no internet access. To install OpenClaw offline:"
            ui_info "  1) On a machine WITH internet, run: npm pack openclaw"
            ui_info "  2) Copy the .tgz file to this machine"
            ui_info "  3) Run: npm install -g ./openclaw-*.tgz"
            exit 1
        else
            ui_info "No network but OpenClaw already installed; proceeding with upgrade check"
        fi
    fi
    ...
    # Step 7: Clean up duplicate lines in shell rc files
    # External tools (openclaw doctor, nvm, etc.) may append duplicate init lines
    deduplicate_rc_files

    # Step 8: If BOOTSTRAP.md is still present in the workspace, resume onboarding
    run_bootstrap_onboarding_if_needed
