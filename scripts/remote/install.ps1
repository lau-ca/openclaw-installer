...
# Setup network mirrors (China-friendly fallback)
function Setup-NetworkMirrors {
    # Respect pre-configured mirrors from parent process (e.g. Tauri app)
    if (-not [string]::IsNullOrWhiteSpace($env:npm_config_registry)) {
        Write-Host "[OK] Using pre-configured npm registry: $env:npm_config_registry" -ForegroundColor Green
        return
    }

    # Quick probe: can we reach npmjs.org?
    $reachable = $false
    try {
    ...
    if (-not $reachable) {
        # Check if mirrors are reachable (distinguish 'blocked' vs 'no network at all')
        $mirrorReachable = $false
        try {
            $req2 = [System.Net.WebRequest]::Create("https://registry.npmmirror.com/")
            $req2.Timeout = 5000
            $req2.Method = "HEAD"
        ...
        if (-not $mirrorReachable) {
            $script:NoNetwork = $true
            Write-Host "[!] No network connectivity detected (npm registry and mirrors both unreachable)" -ForegroundColor Yellow
            Write-Host "    The installer will check for pre-installed tools and skip downloads" -ForegroundColor Gray
            return
        }

        Write-Host "[!] npm registry appears unreachable; using Chinese mirror (npmmirror.com)" -ForegroundColor Yellow
...
function Install-Node {
    ...
    if (Get-Command winget -ErrorAction SilentlyContinue) {
        ...
        try {
            ...
            Write-Host "[!] winget install threw an error: $_" -ForegroundColor Yellow
        }
        ...
        Write-Host "[!] winget install did not result in a working Node.js; trying next method" -ForegroundColor Yellow
    }
    ...
    if (Get-Command choco -ErrorAction SilentlyContinue) {
        ...
        try {
            ...
            Write-Host "[!] Chocolatey install threw an error: $_" -ForegroundColor Yellow
        }
        ...
        Write-Host "[!] Chocolatey install did not result in a working Node.js; trying next method" -ForegroundColor Yellow
    }
    ...
    if (Get-Command scoop -ErrorAction SilentlyContinue) {
        ...
        try {
            ...
            Write-Host "[!] Scoop install threw an error: $_" -ForegroundColor Yellow
        }
        ...
        Write-Host "[!] Scoop install did not result in a working Node.js" -ForegroundColor Yellow
    }

    # Manual download fallback
    Write-Host "" 
    Write-Host "Error: Could not install Node.js via any available package manager" -ForegroundColor Red
    Write-Host ""
...
function Main {
    ...
    # Setup network mirrors if needed
    Setup-NetworkMirrors
    ...
    if (-not (Check-Node)) {
        if ($script:NoNetwork) {
            Write-Host "" 
            Write-Host "Error: Node.js v22+ is required but not installed, and no network is available" -ForegroundColor Red
            Write-Host "Please pre-install Node.js 22+ on this machine before running the installer" -ForegroundColor Yellow
            Write-Host "  Download offline: https://nodejs.org/en/download/" -ForegroundColor Cyan
            exit 1
        }
        Install-Node
    ...
    # Step 2: OpenClaw
    if ($script:NoNetwork) {
        $existingClaw = Get-OpenClawCommandPath
        if (-not $existingClaw) {
            Write-Host "" 
            Write-Host "Error: Cannot install OpenClaw: no network connectivity" -ForegroundColor Red
            Write-Host "This machine has no internet access. To install OpenClaw offline:" -ForegroundColor Yellow
            Write-Host "  1) On a machine WITH internet, run: npm pack openclaw" -ForegroundColor Gray
            Write-Host "  2) Copy the .tgz file to this machine" -ForegroundColor Gray
            Write-Host "  3) Run: npm install -g ./openclaw-<version>.tgz" -ForegroundColor Gray
            exit 1
        } else {
            Write-Host "[!] No network but OpenClaw already installed; proceeding with upgrade check" -ForegroundColor Yellow
        }
    }
